...
string DataSource::tpchSentence(){
	std::cout << "tpchSentence Usage" << std::endl;
...
string DataSource::tpchNounPhrase(){
	std::cout << "tpchNounPhrase Usage" << endl;
	if(randomTrue(1/4))
...
std::string DataSource::outputPath;
int DataSource::warehouseCount = 1;  // 默认值

void DataSource::initialize(const std::string& path, int whCount) {
    srand(1382350201);
    outputPath = path;
...
void DataSource::getRemoteWId(int& currentWId, int& ret){
	if( getWarehouseCount()==1){
		ret = currentWId;
	}
	else{
		ret = currentWId;
		while(ret == currentWId)
			randomUniformInt(1, getWarehouseCount(),ret);
	}
}
