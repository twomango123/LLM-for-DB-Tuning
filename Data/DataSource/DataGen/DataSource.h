...
class DataSource{
	...
	public:
		...
		static void initialize(const std::string& path = "", int whCount = 1);
    
    // 获取方法
		static const std::string& getOutputPath() { return outputPath; }
		static int getWarehouseCount() { return warehouseCount; }
		
		// 单独的设置方法（可选）
		static void setOutputPath(const std::string& path) { outputPath = path; }
		static void setWarehouseCount(int count) { warehouseCount = count; }
		
		// 保持向后兼容的无参版本
		static void initialize() { initialize("", 1); }
