...
if not api_key:
    # 兼容历史：若未提供环境变量，沿用原硬编码值（如有）
    api_key = "sk-Z8af9sbqPZvXIG18uCttyzhDVzpUTH4tuj5huEGJoI2vkvyh"

url = os.getenv("LLM_API_URL", "https://api.aaigc.top/v1/chat/completions")
# 规范化 URL，去除多余空白并支持相对路径拼接
url = (url or "").strip()
if not url.startswith("http"):
    base = (os.getenv("LLM_API_BASE", "https://api.aaigc.top") or "").strip()
...
async def call_llm_api(session: aiohttp.ClientSession, prompt: str , background: str) -> dict:
    ...
    async with session.post(url, headers=headers, data=json.dumps(data)) as resp:
        try:
            ...
            raise RuntimeError(f"LLM API 非 JSON 响应，status={resp.status}, body={text[:256]}")
        if isinstance(response_json, dict) and response_json.get("error"):
            err = response_json.get("error", {})
            msg = err.get("message") or str(err)
            raise RuntimeError(f"LLM API 错误：{msg}")
